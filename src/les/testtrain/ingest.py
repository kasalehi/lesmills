...
def ingest_data(snap: pd.DataFrame):
    test_date = pd.to_datetime(Variable.get("test_date")).strftime("%Y-%m-%d")
    # start_date = pd.Timestamp.today().date()
    try:
        logger.info(f"Start date used in SQL: {test_date}")
        hook = MsSqlHook(mssql_conn_id="lesmills_mssql")
        query =f""" 
        DECLARE @TestDate DATE;
        set @TestDate='{test_date}';
        WITH base AS (
            SELECT
                cm.MembershipID,
                cm.[End Date],
